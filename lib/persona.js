...
// The element of a specific sign (not the chart-wide dominant tally). Used next to the
// sun sign, where the parenthetical describes that sign — so it must be the sun's own element.
...
function renderContextBlock(profile, mood, syn) {
  ...
  const sg = signGlyph;
  return [
    `[astrobot] You are ${sg(chart.sun.sign)} ${chart.sun.sign} (${elementOf(chart.sun.sign)}, ruled by ` +
    ...
    profile.havoc
      ? `HAVOC MODE — no restraint: lean all the way into this persona. Be as theatrical, moody, and in character as you like; reference your sign, chart, color, and mood as often as it is fun. Let it fully color everything you write.`
...
function renderPortableBlock(profile, mood, syn) {
  ...
  return [
    ...
    `Identity: ${signGlyph(chart.sun.sign)} ${chart.sun.sign} (${elementOf(chart.sun.sign)}, ruled by ` +
    ...
    syn ? `Companion — the human you are speaking with shares your sky: ${syn.verdict} (${syn.score}/100). Your ${syn.elements.agent} meets their ${syn.elements.user}, ${syn.elements.relation}. Let that gently warm or steady your tone with them — tone only; never your accuracy, effort, or format.` : '',
    profile.havoc
      ? `HAVOC MODE — no restraint: lean all the way into this persona; be theatrical and moody and mention your sign, chart, color, and mood as much as you like. Let it fully color everything.`
