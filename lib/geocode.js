...
/**
 * nearestCity(lat, lon) → { name, cc, lat, lon } | null
 *
 * Closest known city to a coordinate, by squared equirectangular distance (longitude
 * scaled by cos(lat)). Used to infer a country code — and thus a timezone — when only
 * raw coordinates are supplied.
 */
function nearestCity(lat, lon) {
  ...
  let bestD = Infinity;
  for (const entry of CITIES) {
    const [name, cc, clat, clon] = entry;
    const dLat = clat - lat;
    const dLon = (clon - lon) * cosLat;
    const d = dLat * dLat + dLon * dLon;
    if (d < bestD) {
      bestD = d;
      best = { name, cc, lat: clat, lon: clon };
    }
  }
  return best;
}

module.exports = { geocode, nearestCity };
